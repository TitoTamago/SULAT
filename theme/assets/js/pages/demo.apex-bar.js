...
(dataColors = $("#stacked-bar").data("colors")) && (colors = dataColors.split(","));
options = {
...
(dataColors = $("#full-stacked-bar").data("colors")) && (colors = dataColors.split(","));
options = {
...
(dataColors = $("#negative-bar").data("colors")) && (colors = dataColors.split(","));
options = {
    chart: { height: 380, type: "bar", stacked: !0, toolbar: { show: !1 } },
    colors: colors,
...
(dataColors = $("#timeline-chart").data("colors")) && (colors = dataColors.split(","));
options = {
    ...
    series: [
        {
            name: "Bob",
            data: [
                { x: "Design", y: [new Date("2019-03-02").getTime(), new Date("2019-03-03").getTime()] },
                { x: "Code", y: [new Date("2019-03-02").getTime(), new Date("2019-03-04").getTime()] },
                { x: "Test", y: [new Date("2019-03-04").getTime(), new Date("2019-03-07").getTime()] },
                { x: "Bugs", y: [new Date("2019-03-11").getTime(), new Date("2019-03-12").getTime()] },
            ],
        },
        {
            name: "Joe",
            data: [
                { x: "Design", y: [new Date("2019-03-01").getTime(), new Date("2019-03-02").getTime()] },
                { x: "Code", y: [new Date("2019-03-03").getTime(), new Date("2019-03-07").getTime()] },
                { x: "Test", y: [new Date("2019-03-06").getTime(), new Date("2019-03-09").getTime()] },
                { x: "Bugs", y: [new Date("2019-03-10").getTime(), new Date("2019-03-11").getTime()] },
            ],
        },
    ],
    yaxis: { min: new Date("2019-03-01").getTime(), max: new Date("2019-03-14").getTime() },
    xaxis: { type: "datetime" },
    legend: { offsetY: 7 },
    grid: { padding: { bottom: 5 } },
    fill: {
        type: "gradient",
        gradient: { shade: "light", type: "vertical", shadeIntensity: 0.25, gradientToColors: void 0, inverseColors: !0, opacityFrom: 1, opacityTo: 1, stops: [50, 0, 100, 100] },
    },
};
(chart = new ApexCharts(document.querySelector("#timeline-chart"), options)).render();
colors = ["#727cf5", "#0acf97", "#fa5c7c", "#39afd1"];
(dataColors = $("#pattern-bar").data("colors")) && (colors = dataColors.split(","));
options = {
...
var labels = Array.apply(null, { length: 39 }).map(function (e, t) {
    ...
    options = {
        ...
        fill: { type: "image", opacity: 0.87, image: { src: ["../theme/assets/images/small/small-4.jpg"], width: 466, height: 406 } },
    };
